        ...
        return(
            <div className="w-full h-full relative">
                ...
                {this.state.values.length > 0
                    ? <div onClick={ this.props.move < 3 && ones.length < 3 && this.props.currentOnes + ones.length < 3 ? this.props.handleNewDiceRoll : this.props.newGame } className={ `${ this.state.valuesLoaded ? 'opacity-100' : 'opacity-0 translate-y-4 ' } active:scale-75 active:rotate-45 active:opacity-0 w-full h-8/10 flex content-center gap-y-6 justify-center gap-x-6 flex-wrap mb-2 transition-all duration-200 xs:px-4 sm:px-0` }>
